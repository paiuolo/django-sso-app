...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-sso-app runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-sso-app.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ django_sso_app")
...
def lint(c):
    ...
    """
    c.run("flake8 django-sso-app tests")
...
def release(c, bumpsize=''):
    ...
    import django_sso_app
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_sso_app.__version__))
    c.run("git push --tags")
